...
public class Player2Code : MonoBehaviour {
	...
	void Start()
	{
		...
		theTimer.text = "";
		beginningText.text = "";
		jumpSpeed1 = new Vector3(Input.GetAxis("Horizontal"), 6, Input.GetAxis("Vertical"));
	...
	void FixedUpdate()
	{
		...
		{
			rb.position += jumpSpeed1 * Time.deltaTime;
		}
		...
		if (rb.position.y < -6) {
			rb.position = new Vector3(-1.24f, 2f, 0f);
